...
use crate::modelo::{
    Adjunto, Cuando, Error, Evento, EventoNuevo, Excepcion, Imagen, Importancia, Notificacion,
    FORMATO,
};
...
pub fn borrar(conexion: &Connection, id: i64) -> Result<Accion, Error> {
    ...
    let adjuntos: Vec<Adjunto> = {
        let mut c = tx.prepare("SELECT * FROM adjunto WHERE evento_id = ?1")?;
    ...
    let excepciones: Vec<Excepcion> = {
        let mut c = tx.prepare("SELECT * FROM excepcion WHERE evento_id = ?1")?;
        let filas = c.query_map([id], |f| {
            Ok(Excepcion {
    ...
    let notificaciones = notificaciones_de(&tx, id)?;

    tx.execute("DELETE FROM evento WHERE id = ?1", [id])?;
    tx.commit()?;

    Ok(Accion::EventoBorrado {
        evento,
        adjuntos,
        excepciones,
        notificaciones,
...
pub fn restaurar(
    ...
    insertar_notificaciones(&tx, evento.id, notificaciones)?;

    tx.commit()?;

    Ok(Accion::EventoCreado { id: evento.id })
}
